...
- [Maven](#maven)
  ...
  - [生命周期](#生命周期)
    - [clean生命周期](#clean生命周期)
    - [default生命周期](#default生命周期)
  - [自定义操作到生命周期某个阶段](#自定义操作到生命周期某个阶段)
    - [不同的生命周期一起运行](#不同的生命周期一起运行)
  - [私服和镜像的使用](#私服和镜像的使用)
    - [配置镜像](#配置镜像)
  - [定义不同的配置环境](#定义不同的配置环境)
  - [maven-assembly-plugin打包](#maven-assembly-plugin打包)
    - [配合SpringBoot使用](#配合springboot使用)
...
目前java项目流行的项目管理工具`Maven`和`Gradle`。目前使用率来说，`Maven`使用占比是高于`Gradle`的。纠结选哪个管理工具的时候就用`Maven`就行。

在`Maven`中通过`POM-Project Object Model`项目对象模型的概念来管理项目。在maven中每个项目都相当于一个对象，对象（项目）之间可以依赖、继承、聚合。

项目依赖包查找网站[https://mvnrepository.com/](https://mvnrepository.com/)

依赖包放在`Maven`仓库中，我们需要通过依赖的坐标信息，从仓库中下载jar。

坐标信息：groupId，artifactId，version

当我们在`pom.xml`引入某个`a.jar`，`Maven`会从本地仓库查找有没有`a.jar`，没有就会去私服下载，私服没有会从中央仓库下载到私服，然后下载到本地仓库。

![](.maven_images/861036cc.png)
...
所以maven依赖传递的其中一个原则就是，`最短路径原则`

a->b->c(2.0)

a->c(1.0)

maven-client项目最终使用1.0的c。

#### 最先声明原则

最短路径原则存在一个问题，当最短路径一样的时候怎么办，最先声明原则就是在最短路径相同的时候生效。
...
- 第一种，打破最短依赖路径，直接在maven-client引入你想要的maven-c版本
...
这样排除掉maven-b传递过来的maven-c。根据自己的需求
...
依赖的作用域定义了jar在什么时候生效，打包成war或者打包成jar的时候是否打包进去。
...
scope默认取值compile标识，当前依赖包，参与项目的编译、运行、测试、打包。

#### runtime

runtime标识不参与项目的编译，参与项目的运行、测试、打包。

#### provided

provided表示参与项目的编译、运行、测试，但是不参与打包。

#### system

system用于我们开发的一些二方库，但是不能发布到网上，可以使用这种场景去使用。默认这种jar不会打包进去，需要我们配置一下。
...
test标识只在编译测试代码和运行测试代码的时候需要，别的都不用，打包的时候也不会包含。

#### import

一般我们会在父工程使用这个作用域。表示从其他pom.xml导入dependency的配置。

比如我们自己的父工程，只想要springboot的依赖管理。
...
聚合之前打包a,b,c,d,client需要分别运行mvn package。

聚合之后，只需要在maven-demo下运行mvn package。

### 生命周期

maven内部有三个构建周期。clean，default，site。

其中default是我们经常使用的生命周期。
...
resources：拷贝src/main/resources下资源到classpath下。

compile：编译项目源码

testResources：拷贝src/test/resources到测试的test的classpath下
...
deploy：发布jar到远程仓库或私有仓库

![图 1](.maven_images/ddd682f9c9d149d9a32133ed8a0a4ffea768f527eeb7240aaf694507e0675bd4.png)  
...
先执行clean生命周期，再执行default生命周期到package阶段。

### 私服和镜像的使用

#### 配置镜像

mirrorOf指定代理那个仓库setting.xml配置镜像
...
也可以根据与不同的配置环境，启动不同jdk环境，或者根据不同的环境启动不同的镜像
